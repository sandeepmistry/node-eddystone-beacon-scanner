...
var uriDecode = require('eddystone-url-encoding').decode;
...
EddystoneBeaconScanner.prototype.onDiscover = function(peripheral) {
  ...
  if (this.isUrlBeacon(peripheral)) {
    var urlBeacon = this.parseUrlBeacon(peripheral);

    debug('onDiscover: urlBeacon = %s', JSON.stringify(urlBeacon));
    this.emit('discover', urlBeacon);
  }
};

EddystoneBeaconScanner.prototype.isUrlBeacon = function(peripheral) {
